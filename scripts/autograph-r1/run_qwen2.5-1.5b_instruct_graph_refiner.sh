...
if [ "$MIX_DATA" = "True" ] && [ "$ITERATIVE" = "False" ]; then
    ...
    if [ "$REFINEMTN" = "True" ]; then
        TRAIN_DATA="/data/haoyuhuang/data/AtlasTune/data/mixed_hotpot_musique_train_doc_size_15_distract_${WITH_DISTRACT}_iterate_refinement.parquet"
        VAL_DATA="/data/haoyuhuang/data/AtlasTune/data/mixed_hotpot_musique_valid_doc_size_15_distract_${WITH_DISTRACT}_iterate_refinement.parquet"
    else
...
CHECKPOINT_DIR="/data/haoyuhuang/data/AtlasTune/checkpoints/${TIMESTAMP}_qwen2.5-1.5B-refinement-${DIFFFICULTY}-docsize${DOC_SIZE}-textlinking${TEXT_LINKING}-loose"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="azure-qwen2.5-1.5B-refinement-rl-distract-${DIFFFICULTY}-docsize${DOC_SIZE}-text-linking${TEXT_LINKING}-deduce-${DEDUCE_REWARD}-loose" \
